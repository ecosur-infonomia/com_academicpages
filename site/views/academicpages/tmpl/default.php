...
<script id="datos-personales-template" type="text/x-handlebars-template">
    ...
    <dl>
        <dt><a href="<?php echo (JURI::base());?>?option=com_areasacademic&area={{deptId}}">{{dept}}</a></dt>
        <dd>Grupo: {{grupo}}</dd>
        {{#if asociados}}
...
<script id="rel-template" type="text/x-handlebars-template">
    <ul>
        {{#each related}}
        <li><a href="<?php echo (JURI::base());?>?option=com_academicpages&view=academicpages&id={{this.IDPersona}}&Itemid=1457"><i class="icon-user"></i>&nbsp;{{#if this.Titulo}}{{this.Titulo}}{{/if}}{{this.NombreAbreviado}}</a></li>
        {{/each}}
    </ul>
</script>
